...
def submit_expence (request):
    ...
    if form.is_valid():
        ...
        form_token = form.cleaned_data.get('token')
        ...
        form_amount = form.cleaned_data.get('amount')
        form_user = get_object_or_404(User,token__token=form_token)
        print('this form = ', form_token,' ',form_text,' ',form_user,' ',form_amount)
        Expence.objects.create(user=form_user,amount=form_amount,text=form_text,date=date)



    context={"form":form,"title":"Submit Expence","submit":"submit"}
    return render(request,'submit_expence.html',context)
